...
class Home extends React.Component {
  ...
  componentDidMount() {
    ...
    queryworldcubstate(localStorage.getItem("openid"), 1).then(res => {
  ...
  goInfo = (num) => {
    ...
    if (num == 3) {
      awardstate == 2 ? Toast.fail("未到开奖时间") : this.props.history.push("/result")
      return false
    }
    if (num == 1) {
    ...
    if (num == 2) {
      awardstate == 2 ? this.setState({ modal1: true }) : ""
      return false
    }
      ...
      return (
        <div className={styles.normal} >
          ...
          <div className={styles.btnContent}>
            <div className={styles.btn} onClick={() => { this.goInfo(1) }}>小组赛：我的竞猜</div>
            <div className={styles.btn2} onClick={() => { this.goInfo(2) }}>淘汰赛：参与竞猜</div>
            <div className={styles.btn} onClick={() => { this.goInfo(3) }}>中奖名单</div>
          </div>
          <Modal
